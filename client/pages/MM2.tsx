...
type FilterKey = "all" | "pets" | "plants" | "best" | "bundles" | "sheckle" | "fruits" | "mutated" | "mega" | "summer" | "knives" | "guns";

type CartLine = { id: string; qty: number; item: Item };

// Catalog (Murder Mystery 2)
export const ITEMS: Item[] = [
  { id: "chroma-blade", name: "Chroma Blade", image: "https://cdn.builder.io/api/v1/image/assets%2F63c936af87bb4092b7300f333f376cfe%2F205ad800b7024ca2b7155e38798bae3b?format=webp&width=800", category: "pet", stock: 5, priceUSD: 15, tags: ["best","knives"] },
  { id: "tree-knife", name: "Tree Knife", image: "https://cdn.builder.io/api/v1/image/assets%2F63c936af87bb4092b7300f333f376cfe%2F6423c1dd754049b1a5cffcc85838ee79?format=webp&width=800", category: "pet", stock: 4, priceUSD: 12, tags: ["knives"] },
  ...
  { id: "tree-gun", name: "Tree Gun", image: "https://cdn.builder.io/api/v1/image/assets%2F63c936af87bb4092b7300f333f376cfe%2F5e22f58e53294ed6a26f2c6ab50df3d6?format=webp&width=800", category: "pet", stock: 3, priceUSD: 14, tags: ["guns"] },
  ...
  { id: "chroma-luger", name: "Chroma Luger", image: "https://cdn.builder.io/api/v1/image/assets%2F63c936af87bb4092b7300f333f376cfe%2F12b3665e7d564c778d3163afd561a555?format=webp&width=800", category: "plant", stock: 5, priceUSD: 22, tags: ["guns","summer"] },
];
...
export default function MM2() {
  ...
  useEffect(() => {
    try { localStorage.setItem('app:cart', JSON.stringify(cart || [])); window.dispatchEvent(new CustomEvent('cart:update', { detail: cart })); } catch {}
  }, [cart]);
  useEffect(() => {
    const onStorage = (e: StorageEvent) => { if (e.key === 'app:cart') { try { setCart(JSON.parse(e.newValue || '[]')); } catch {} } };
    window.addEventListener('storage', onStorage);
    const onUpdate = (e: Event) => { try { const detail = (e as CustomEvent<Item[]>)?.detail; if (Array.isArray(detail)) setCart(detail); } catch {} };
    window.addEventListener('cart:update', onUpdate as EventListener);
    return () => { window.removeEventListener('storage', onStorage); window.removeEventListener('cart:update', onUpdate as EventListener); };
  }, []);
  ...
  // Load admin custom products and deleted ids for MM2
  useEffect(() => {
    const keyC = "admin:customProducts:mm";
    const keyD = "admin:deletedIds:mm";
    const load = () => {
      try {
  ...
  const itemsForFilter = (key: FilterKey): Item[] => {
    ...
    else if (key === "summer") list = list.filter((i) => i.tags?.includes("summer"));
    else if (key === "knives") list = list.filter((i) => i.tags?.includes("knives"));
    else if (key === "guns") list = list.filter((i) => i.tags?.includes("guns"));
    return list;
  };
            ...
            <div className="hidden md:block w-full rounded-2xl border border-emerald-700/20 bg-emerald-900/20 p-4 fixed top-16 left-0 right-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-background/60">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" className={`px-2 py-2 text-white ${filter === "summer" ? "underline" : ""}`} onClick={() => setFilter("summer")}>
                    Summer Specials
                  </Button>
                  <Button variant="ghost" size="sm" className={`px-2 py-2 text-white ${filter === "knives" ? "underline" : ""}`} onClick={() => setFilter("knives")}>
                    Knives
                  </Button>
                  <Button variant="ghost" size="sm" className={`px-2 py-2 text-white ${filter === "guns" ? "underline" : ""}`} onClick={() => setFilter("guns")}>
                    Guns
                  </Button>
          ...
          <div className="md:hidden fixed bottom-0 left-0 right-0 z-[120] border-t border-white/10 bg-background/90 backdrop-blur">
            <div className="container py-2 best-scroll-wrapper relative">
              ...
              <div ref={bottomScrollRef} className="best-scroll flex items-center px-2 py-2 gap-1 overflow-x-auto" onWheel={(e) => { const el = e.currentTarget as HTMLElement; if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) { e.preventDefault(); e.stopPropagation(); el.scrollLeft += e.deltaY * 3; } }}>
                {[
                  { key: 'best', label: 'Best Sellers', color: 'text-red-400 hover:bg-gradient-to-t hover:from-red-500/10 hover:text-red-300' },
                  { key: 'bundles', label: 'Bundles', color: 'text-gray-400 hover:bg-gradient-to-t hover:from-emerald-500/10 hover:text-emerald-300' },
                  { key: 'summer', label: 'Summer', color: 'text-gray-400 hover:bg-gradient-to-t hover:from-emerald-500/10 hover:text-emerald-300' },
                  { key: 'knives', label: 'Knives', color: 'text-gray-400 hover:bg-gradient-to-t hover:from-emerald-500/10 hover:text-emerald-300' },
                  { key: 'guns', label: 'Guns', color: 'text-gray-400 hover:bg-gradient-to-t hover:from-emerald-500/10 hover:text-emerald-300' },
                ].map((b) => (
                  ...
                  >
                    <div className="mb-1 flex items-center justify-center">
                      <div className="scale-75">
                        <svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <rect width="42" height="42" rx="10" fill={b.key === 'best' ? '#FE5050' : '#22c55e'} fillOpacity="0.1"></rect>
                          <g transform="translate(4, 4)">
                            {b.key === 'best' && (
                              <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 24 24" fill="none" stroke="#FE5050" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6">
                            ...
                            {b.key === 'bundles' && (
                              <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6">
                                <path d="M19.5 12.572l-7.5 7.428l-7.5 -7.428a5 5 0 1 1 7.5 -6.566a5 5 0 1 1 7.5 6.572"></path>
                              </svg>
                            )}
                            {b.key === 'summer' && (
          ...
          <div className="grow-grid mt-2 grid grid-cols-[120px,1fr] gap-4 md:mt-0 md:block">
            <aside className="mobile-filters hidden md:hidden rounded-2xl border border-white/10 bg-slate-900/60 p-2 min-h-[calc(100vh-6rem)]">
              <div className="sticky top-20 space-y-2">
                <div className="flex items-center gap-2 rounded-md bg-slate-900/80 text-white border border-white/10 px-2 py-1">
                  <img src="https://cdn.builder.io/api/v1/image/assets%2Fb2458d5aab5847128a2b754080dc1712%2F155ecd3b276d4626afdb8f1be5054597?format=webp&width=400" alt="Murder Mystery 2" className="h-7 w-10 rounded object-cover" />
                  <span className="font-semibold text-xs sm:text-sm">Murder Mystery 2</span>
                </div>
                ...
                <div className="grid gap-2">
                  ...
                  <Button className="filter-custom justify-start w-full px-3 py-2 bg-transparent hover:bg-transparent border-0 shadow-none text-white" size="sm" onClick={() => setFilter("summer")}>
                    Summer Specials
                  </Button>
                  <Button className="filter-custom justify-start w-full px-3 py-2 bg-transparent hover:bg-transparent border-0 shadow-none text-white" size="sm" onClick={() => setFilter("knives")}>
                    Knives
                  </Button>
                  <Button className="filter-custom justify-start w-full px-3 py-2 bg-transparent hover:bg-transparent border-0 shadow-none text-white" size="sm" onClick={() => setFilter("guns")}>
                    Guns
                  </Button>
            ...
            {(() => {
              if (filter === "all") {
                const order: FilterKey[] = ["best", "bundles", "summer", "knives", "guns"];
                return (
                  <div>
                    {order.map((key) => {
          ...
          <div className="md:hidden fixed inset-0 z-[2147483651] bg-gradient-to-b from-[#0b0d1a] to-[#111221] border border-white/5">
            <div className="flex h-full flex-col">
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                ...
                <button onClick={() => setCartOpen(false)} className="h-9 w-9 rounded-md bg-emerald-500 hover:bg-emerald-600 flex items-center justify-center text-white">×</button>
              </div>
