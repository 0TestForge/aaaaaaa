...
import { ITEMS } from "./GrowGarden";
import { useGeo } from "@/hooks/useGeo";
import { useToast, toast } from "@/hooks/use-toast";

export default function GrowGardenAll() {
  ...
  const priceFmt = useMemo(
    () => new Intl.NumberFormat("en-US", { style: "currency", currency: geo.currency || "USD", currencyDisplay: "narrowSymbol", minimumFractionDigits: 0, maximumFractionDigits: 0 }),
    [geo.currency]
  );
  const toLocal = (usd: number) => usd * rate;
  const toLocalRounded = (usd: number) => Math.round(toLocal(usd));

  // Admin overrides / custom products / deletions
  ...
  useEffect(() => {
    ...
    const onPrices = () => {
      try { const raw = localStorage.getItem("priceOverrides"); setOverrides(raw ? JSON.parse(raw) : {}); } catch {}
    };
  ...
  // Cart state that mirrors app behavior
  ...
  const addToCart = (item: any) => {
    setCart((c) => [...c, item]);
    try { window.dispatchEvent(new CustomEvent('cart:update', { detail: cart })); } catch {}
    try { /* removed toast */ } catch {}
  };
