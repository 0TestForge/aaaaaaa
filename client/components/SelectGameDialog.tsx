...
export function SelectGameDialog({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent
        className="w-[92vw] max-w-md sm:max-w-3xl max-h-[85vh] overflow-auto sm:overflow-hidden bg-cover bg-center p-4 sm:p-6"
        style={{
          backgroundImage:
      ...
      >
        <div className="p-6 pt-4">
          <DialogHeader className="-mt-1">
            <DialogTitle className="text-2xl md:text-3xl font-extrabold tracking-wider uppercase text-foreground drop-shadow-sm">Choose a Game</DialogTitle>
          </DialogHeader>
          <div className="mt-2 grid grid-cols-3 gap-2 sm:grid-cols-2 sm:gap-3 md:grid-cols-3">
            {items.map((it) => (
              <Button
                key={it.id}
                className="group w-full rounded-2xl border border-white/10 bg-white/5 p-2 sm:p-3 hover:bg-white/10 text-center sm:text-left flex flex-col items-center gap-2 sm:flex-row sm:items-center sm:gap-3 h-auto sm:h-28"
                onClick={() => {
              ...
              >
                <span className="relative inline-flex h-14 w-16 sm:h-20 sm:w-24 shrink-0 overflow-hidden rounded-xl border border-white/10">
                  <img src={it.image} alt={`${it.name} cover`} className="h-full w-full object-cover" />
                </span>
                <span className="sm:ml-1">
                  <span className="block text-xs sm:text-base font-semibold leading-tight">{it.name}</span>
                  <span className="mt-0.5 hidden sm:block text-xs text-muted-foreground">Tap to view items</span>
                </span>
              </Button>
            ))}
          </div>
